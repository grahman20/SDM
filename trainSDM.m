function [svm_model, kernel_param, training_features] = trainSDM(pos_features, neg_features, ul_features, param, options)
% function [model models]= trainSDM(pos_features, neg_features, ul_features, param)
% Input:
...
for i = 1:MAX_ITER
    ...
    % train SVM
    vparam.kernel_type = param.kernel_type; %'gaussian';
    [training_kernel,kernel_param] = getKernel(training_features, vparam);
    %added by Gea: calculate MMD (M) and Manifold (L) and integrate with SVM kernel
    if i>1
        Xs = [s_pos_feats s_neg_feats];
        Ys = [ones(pos_sn, 1); -ones(neg_sn, 1)];
        Xt = [t_pos_feats t_neg_feats];
        Yt = [ones(pos_tn, 1); -ones(neg_tn, 1)];
        Xs_transpose=Xs';
        Xt_transpose=Xt';
        [M, L] = getML(Xs_transpose,Ys,Xt_transpose,Yt,options);
%         n = size(Xs,2);
%         m = size(Xt,2);
...
%         end
%         YY = [YY;-ones(m,CC)];
%         training_kernel=((E + options.lambda * M + options.rho * L) * training_kernel+ options.eta * speye(n + m,n + m))\E;% (E * YY);
        training_kernel=(options.lambda * M + options.rho * L) * training_kernel;
        training_kernel=abs(training_kernel./diag(training_kernel));
%          training_kernel=training_kernel./diag(training_kernel);
    end
